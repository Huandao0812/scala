...
abstract class TweetSet {
   ...
   * and be implemented in the subclasses?
   */
  def mostRetweetedHelper(currentMax: Tweet): Tweet 
...
class Empty extends TweetSet {
  ...
  def mostRetweetedHelper(currentMax: Tweet): Tweet = currentMax
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedHelper(currentMax: Tweet): Tweet = 
	if(elem.retweets >= currentMax.retweets) right mostRetweetedHelper(left mostRetweetedHelper(elem))
	else right mostRetweetedHelper(left mostRetweetedHelper(currentMax))
...
object GoogleVsApple {
  ...
  val allT = TweetReader.allTweets
  lazy val googleTweets: TweetSet = allT.filter(tweet => google.exists(ele => tweet.text.contains(ele)))
  lazy val appleTweets: TweetSet = allT.filter(tweet => apple.exists(ele => tweet.text.contains(ele)))
